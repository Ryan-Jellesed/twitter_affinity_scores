...
def gather_friends(api, ids, friend_limit = 10000, file_name=None) :
    ''' from a set of IDs, get all the friends of those ids up to friend
        limit. Returns a dictionary with key = id, value = follower_id list
        THIS FUNCTION RYAN ADDED TO TRY AND PULL THE FRIENDS OF A FOLLOWER
    '''
    ...
    for this_id in ids :
        ...
        try :
            for page in tweepy.Cursor(api.friends_ids,user_id = this_id).pages() :
                ...
                if friend_limit and pulled_friends > friend_limit :
                     break
